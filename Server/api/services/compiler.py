...
def compile_python(code, input_data):
    try:
        ...
        result = subprocess.run(
            ["python", "-c", code], input=input_data, capture_output=True, text=True
        )
...
def compile_c(code, input_data):
    try:
        ...
        run_result = subprocess.run(
            ["./temp.out"], input=input_data, capture_output=True, text=True
        )
...
def compile_cpp(code, input_data):
    try:
        ...
        compile_result = subprocess.run(
            ["g++", "temp.cpp", "-o", "temp.out"], capture_output=True, text=True
        )
        if compile_result.returncode != 0:
            return Response({"error": compile_result.stderr})
...
def compile_java(code, input_data):
    try:
        ...
        run_result = subprocess.run(
            ["java", class_name], input=input_data, capture_output=True, text=True
        )
...
def compile_javascript(code, input_data):
    try:
        ...
        run_result = subprocess.run(
            ["node", "temp.js"], input=input_data, capture_output=True, text=True
        )
