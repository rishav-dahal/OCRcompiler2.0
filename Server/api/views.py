...
def upload_image(request):
    if "image" not in request.data:
        return Response(
            {"msg": "No image in the request"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if image and allowed_file(image.name):
        ...
        data = {"original_image": path, "ocr_text": ocr_text}
    ...
    return Response(
        {"msg": "Image upload failed. Invalid file format."},
...
def download_snippet(request, snippet_id):
    ...
    file_extension = {"python": "py", "java": "java", "cpp": "cpp", "c": "c"}.get(
        snippet.language, "txt"
    )

    response = HttpResponse(snippet.formatted_code, content_type="text/plain")
    response["Content-Disposition"] = (
        f"attachment; filename=snippet_{snippet.id}.{file_extension}"
    )
    return response
...
def delete_code_snippet(request, pk):
    try:
        ...
        return Response(
            {"msg": "Code snippet does not exist"}, status=status.HTTP_404_NOT_FOUND
        )

    permission = IsSnippetOwner()
    if not permission.has_object_permission(request, None, snippet):
        return Response(
            {"msg": "You do not have permission to delete this snippet"},
